...
class CriticNetwork:
    ...
    def create_q_network(self):
        ...
        W_action = self.variable([action_dim, 512], h_conv3_out_size + action_dim)
        ...
        params = [W_conv1, b_conv1, W_conv2, b_conv2, W_conv3, b_conv3, W_fc1, b_fc1, W_fc2, b_fc2, W_action]
    ...
    def create_target_q_network(self, net):
        ...
        h_fc1 = tf.nn.relu(tf.matmul(h_conv3_flat, target_net[6]) + tf.matmul(action_input, target_net[10]) + target_net[7])
        q_value_output = tf.identity(tf.matmul(h_fc1, target_net[8]) + target_net[9])
